      ...
      async (position) => {
        ...
        try {
          ...
          if (data.status === 'OK' && data.results.length > 0) {
            ...
            const suburbResult = data.results.find((result: any) =>
              result.types.includes('locality') || result.types.includes('postal_town')
            );
  
            const fallbackResult = data.results[0];
  
            const address = suburbResult?.formatted_address || fallbackResult?.formatted_address;
  
            if (address) {
              setLocationTerm(address);
            } else {
              console.warn('No suburb or address found for current location');
            }
          } else {
