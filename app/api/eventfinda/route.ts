...
export async function GET(request: Request) {
  try {
    ...
    const response = await fetch(url, {
      headers: {
        'Authorization': 'Basic ' + Buffer.from(EVENTFINDA_USERNAME + ':' + EVENTFINDA_PASSWORD).toString('base64')
      }
    });
    ...
    const events = data.events.map((event: any) => ({
      ...
      logo: event.images?.length > 0 ? {
        url: event.images[0].transforms?.find((t: any) => t.transformation === 'large')?.url || event.images[0].url
      } : undefined
    }));
