...
import { languageToCountry, flagColors, nativeNames } from '@/lib/constants/languageConstants';
...
const EnglishProficiencyChart = ({ lgaCode, lgaName }: LanguageProficiencyChartProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`/api/lga/englishProficiency?lgaCode=${lgaCode}`);
        if (!response.ok) {
        ...
        const languages = Array.from(new Set(data.map(d => d.language))).sort();
  ...
  useEffect(() => {
    if (!selectedLanguage || !svgRef.current || !languageData.length) return;

    // Clear previous chart
    d3.select(svgRef.current).selectAll('*').remove();

    const filtered = languageData.filter(d => d.language === selectedLanguage);
    ...
    // Color scale
    const cc = languageToCountry[selectedLanguage as keyof typeof languageToCountry];
    const sliceColors = (cc && flagColors[cc as keyof typeof flagColors]) ? flagColors[cc as keyof typeof flagColors] : d3.schemeCategory10;
    const color = d3.scaleOrdinal<string>()
      .domain(data.map(d => d.name))
      .range(sliceColors);

    // Determine line color based on flag colors
    const lineColor = (cc && flagColors[cc as keyof typeof flagColors] && flagColors[cc as keyof typeof flagColors].length > 2) ? 'white' : 'black';
    ...
    // Add hover effects
    paths.on('mouseover', function(event, d: d3.PieArcDatum<PieData>) {
      d3.select(this)
        .transition()
        .duration(200)
  ...
  return (
    <div className="w-full max-w-[800px] mx-auto p-4">
      ...
      <div className="mb-2">
        <label htmlFor="langSel" className="block text-sm font-medium text-gray-700 mb-2">
          Select a language:
        </label>
        <select
          id="langSel"
        ...
        >
          {availableLanguages.map(lang => (
            <option key={lang} value={lang}>
              {lang} {nativeNames[lang as keyof typeof nativeNames] || ''}
            </option>
          ))}
        </select>
      </div>
