      ...
      <div className={`w-full ${selectedLgaCode && mode !== 'statistics' && chartData.length > 0 ? 'grid grid-cols-2 gap-4' : 'flex flex-col gap-4'}`}>
        <div className={`${selectedLgaCode && mode !== 'statistics' && chartData.length > 0 ? 'h-[600px]' : 'w-full h-[600px]'} rounded-lg overflow-hidden shadow-lg relative`}>
          <Map
            ...
            interactiveLayerIds={showLandmarks ? [] : ['vic-lga-cleaned-5yz92t']}
          >
            {/* StatisticsMap and LGA highlight layers only when not showing landmarks */}
            {!showLandmarks && mode === 'statistics' && selectedStatistic && (
            ...
            {showLandmarks && landmarks.length > 0 && (
              <>
                ...
                {selectedLandmark && (
                  ...
                  >
                    <LandmarkContent
                      name={selectedLandmark.landmark_name}
                      description={selectedLandmark.landmark_description}
                      website={selectedLandmark.ilms_url}
                      type={selectedLandmark.type?.landmark_type || ''}
                    />
                  </Popup>
                )}
              </>
